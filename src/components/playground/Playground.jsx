...
export default function Playground() {
  ...
  const handleSaveNewVariable = async () => {
    ...
    try {
      const updatedEnv = addVariableToEnvironment(selectedEnvironmentForVariable, {
        key: newVariableKey,
  ...
  const handleSaveEditedVariable = async () => {
    if (!newVariableKey.trim() || selectedVariableIndex === null || !selectedEnvironmentForVariable) return;
    
    try {
      const updatedEnv = updateVariableInEnvironment(selectedEnvironmentForVariable, selectedVariableIndex, {
        key: newVariableKey,
        value: newVariableValue,
        description: newVariableDescription
      });
  ...
  const [editingEnvironmentVariable, setEditingEnvironmentVariable] = useState(null);
  ...
  const [selectedEnvironmentForVariable, setSelectedEnvironmentForVariable] = useState(null);
  ...
  const setRequest = (newRequest) => {
    ...
    if (currentTab && currentTab.request && isAutoGeneratedName(currentTab.name)) {
  ...
  const handleSaveToCollection = async (collectionId) => {
    ...
    try {
      ...
      if (isAutoGeneratedName(requestName) && pendingRequestToSave.request?.url) {
        requestName = generateRequestName(pendingRequestToSave.request.method, pendingRequestToSave.request.url);
      }
      ...
      setRequestTabs(prev => prev.map(tab => 
        tab.id === pendingRequestToSave.id 
          ? { ...tab, name: savedRequest.name, collectionRequestId: savedRequest.id, isModified: false }
          : tab
      ));
  ...
  return (
    ...
    >
                      ...
                      <>
                        ...
                        <div className="mb-4">
                          <Select value={activeEnvironmentId} onValueChange={handleEnvironmentSelect}>
                            <SelectTrigger className={`h-8 text-sm ${themeClasses.input.base}`}>
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              {Object.values(environments).map((env) => (
                                <SelectItem key={env.id} value={env.id}>
                                  <div className="flex items-center gap-2">
                                    <div className={`w-2 h-2 rounded-full bg-${env.color || 'blue'}-500`}></div>
                                    {env.name}
                                  </div>
                                </SelectItem>
                              ))}
                            ...
                            {environmentsArray.map((env) => {
                              ...
                              return (
                                <div key={env.id} className="space-y-1">
                                  ...
                                  <div
                                    className={`group relative flex items-center gap-2 py-2 px-3 transition-all duration-200 cursor-pointer hover:${isDark ? 'bg-gray-800/30' : 'bg-gray-100/50'} rounded-lg ${
                                      isActive ? (isDark ? 'bg-blue-500/10 border border-blue-500/20' : 'bg-blue-50 border border-blue-200') : ''
                                    }`}
                                  >
                                    {/* Expand/Collapse Chevron */}
                                    <button
                                      ...
                                      {editingEnvironment === env.id ? (
                                        ...
                                        <div className="flex items-center justify-between">
                                          <div className={`text-sm font-medium ${
                                            isActive ? themeClasses.text.accent : themeClasses.text.primary
                                          } truncate`}>
                                            {env.name}
                                          </div>
                                    ...
                                    <DropdownMenu>
                                      ...
                                      <DropdownMenuContent align="end" className="w-48">
                                        <DropdownMenuItem onClick={() => handleEnvironmentSelect(env.id)}>
                                          <Check className="h-4 w-4 mr-2" />
                                          Set as Active
                                        </DropdownMenuItem>
                                        <DropdownMenuItem onClick={() => setEditingEnvironment(env.id)}>
                                        ...
                                        >
                                          ...
                                          <div className="flex-1 min-w-0">
                                            <div className={`text-xs font-mono ${themeClasses.text.primary} truncate`}>
                                              {variable.key.toLowerCase().includes('key') || variable.key.toLowerCase().includes('token') || variable.key.toLowerCase().includes('secret') 
                                                ? '••••••••••••' 
                                                : variable.value}
                                            </div>
                                            {variable.description && (
                                              <div className={`text-xs ${themeClasses.text.tertiary} opacity-60 truncate`}>
                                                {variable.description}
                                              </div>
                                            )}
                                          </div>
                                          ...
                                          <div className="opacity-0 group-hover:opacity-100 flex gap-1">
                                            <button
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                handleEditVariable(env.id, index, variable);
                                              }}
                                              className={`p-1 rounded transition-all duration-200 ${themeClasses.button.ghost}`}
                                              title="Edit variable"
                                            >
                                            ...
                                            <button
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                handleDeleteVariable(env.id, index);
                                              }}
              ...
              <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 relative">
                    ...
                    {/* Variable Indicator */}
                    {(() => {
                      const variables = (request.url || '').match(/\{\{([^}]+)\}\}/g);
                        ...
                        return (
                          <div className="absolute right-2 top-1/2 transform -translate-y-1/2">
                            <div 
                              ...
                              title={`Variables: ${variables.map(v => v.replace(/\{\{|\}\}/g, '')).join(', ')}`}
                            >
                    ...
                    <Select value={activeEnvironmentId} onValueChange={handleEnvironmentSelect}>
                      ...
                      <SelectContent>
                        {Object.values(environments).map((env) => (
                          <SelectItem key={env.id} value={env.id}>
                            <div className="flex items-center gap-2">
                              <div className={`w-2 h-2 rounded-full bg-${env.color || 'blue'}-500 flex-shrink-0`}></div>
      ...
      <Dialog open={createEnvironmentDialogOpen} onOpenChange={setCreateEnvironmentDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base font-medium">Create Environment</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={addVariableDialogOpen} onOpenChange={setAddVariableDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base font-medium">Add Variable</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 pt-3">
            <div>
              <Input
            ...
            <div>
              <Input
                ...
                className="h-8 text-sm border-gray-200 focus:border-gray-300 focus:ring-0"
                type={newVariableKey.toLowerCase().includes('key') || newVariableKey.toLowerCase().includes('token') || newVariableKey.toLowerCase().includes('secret') ? 'password' : 'text'}
              />
            </div>
            <div>
              <Input
      ...
      <Dialog open={editVariableDialogOpen} onOpenChange={setEditVariableDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base font-medium">Edit Variable</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 pt-3">
            <div>
              <Input
